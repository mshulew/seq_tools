...
def zipfiles(output_file):
    print('zipping file {} ...'.format(output_file))
    cmd = 'gzip {}'.format(output_file)
...
# write to file
                ...
                with open(r1_output_filename, 'w') as f:
                    for entry in r1_fastq:
                        f.write('%s\n' % entry)   
                output_filenames.append(r1_output_filename)
# zip file
                #print('zipping file...')
...
# write to file
                ...
                output_filenames.append(r1_output_filename)
                output_filenames.append(r2_output_filename)
                
                print('*'*10)
                                   
# multiprocessing to gzip files
    print('zipping files...')
    builtin_outputs = map(zipfiles, output_filenames)
    pool_size = multiprocessing.cpu_count()
    pool = multiprocessing.Pool(processes=pool_size)
    
    pool_outputs = pool.map(zipfiles, output_filenames)
